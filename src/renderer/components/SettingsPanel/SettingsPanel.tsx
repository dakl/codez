...
export function SettingsPanel() {
  ...
  useEffect(() => {
    ...
    window.electronAPI.getMistralApiKey().then((apiKey) => {
      setMistralApiKey(apiKey ?? "");
      setMistralApiKeyInput(apiKey ?? "");
    });
  }, [settingsOpen]);
  ...
  const saveMistralApiKey = useCallback((apiKey: string) => {
    if (!window.electronAPI) return;
    window.electronAPI.setMistralApiKey(apiKey);
    setMistralApiKey(apiKey);
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={closeSettings}>
      ...
      >
        ...
        {/* Mistral API section */}
        <div className="mt-6">
          <h3 className="text-sm font-medium text-text-secondary mb-3">Mistral API</h3>
          ...
          <label className="block mb-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(event) => setMistralApiKeyInput(event.target.value)}
                placeholder="Enter your Mistral API key"
                className="flex-1 rounded-md bg-surface border border-border-subtle px-3 py-1.5 text-xs text-text-primary placeholder:text-text-muted/40 focus:outline-none focus:ring-1 focus:ring-accent"
              />
              <button
                type="button"
                onClick={() => saveMistralApiKey(mistralApiKeyInput)}
                disabled={!mistralApiKeyInput.trim()}
                className="px-3 py-1.5 rounded-md text-xs bg-surface border border-border-subtle text-text-secondary hover:text-text-primary disabled:opacity-40 disabled:cursor-default cursor-pointer transition-colors"
              >
                Save
              </button>
